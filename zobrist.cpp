...
int getPieceValue(int piece)
{
  ...
  if(piece==whitePawn)return 1;
  if(piece==blackKnight)return 2;
  if(piece==whiteKnight)return 3;
  if(piece==blackBishop)return 4;
  if(piece==whiteBishop)return 5;
  if(piece==blackRook)return 6;
  if(piece==whiteRook)return 7;
  if(piece==blackQueen)return 8;
  if(piece==whiteQueen)return 9;
  if(piece==blackKing)return 10;
